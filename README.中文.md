...
max_idle_seconds = 300.0             # 空闲 session 超过此时间后被清理
...
- CORS 可通过 `[cors].*` 配置；为了兼容旧行为，默认仍较宽松，但面向浏览器暴露时应收紧 `origins`。
...
| `response_format` | dict | 控制输出格式。`{"type":"json_object"}` 指示模型仅响应有效 JSON。`{"type":"json_schema","json_schema":{...}}` 提供 JSON Schema 以约束输出。此参数仅在代理层使用，不会转发给 DeepSeek。 |
| `stop` | string \| array | 模型停止生成的序列。当在流中检测到 stop 序列时，输出会在该序列本身之前被截断。支持单个字符串或字符串数组。此参数仅在代理层使用，不会转发给 DeepSeek。 |
| `stream_options` | dict | 流选项，支持 `include_usage` (boolean) 在流式响应中包含 token 使用统计。 |
...
`stop` 参数在代理层消费，通过流式截断实现。当检测到 stop 序列时，代理立即停止向客户端 yield 新内容，同时继续消费 DeepSeek 的 SSE 流以确保连接正确关闭。

核心机制：
- 使用 `stream_buff` 滑动窗口缓冲输出
- Buffer 大小动态计算：`max(len("[TOOL🛠️]"), max(stop 序列长度)) * 2`
- Stop 检测前置到所有 yield 点，保证 stop 序列本身不被输出
- 复用工具调用解析中的 `force_end` 机制

### 流选项（stream_options 参数）

当使用 `stream: true` 进行流式传输时，可以选择提供 `stream_options` 来控制流式行为：
...
| 选项 | 类型 | 说明 |
|------|------|------|
| `include_usage` | boolean | 设为 `true` 时，每个流式 chunk 包含 `usage: null` 字段。此外，在 `data: [DONE]` 前会发送一个包含 token 使用统计的最终 chunk（`choices: []`）。默认为 `false`。 |

> **注意**：Token 使用统计（`prompt_tokens`、`completion_tokens`、`total_tokens`）始终为零，因为 DeepSeek 的流式 SSE 不提供 token 计数。

### 流式响应格式

当 `include_usage: true` 时，流式响应包含额外的 `usage` 字段：

**常规 chunk**（每个 chunk）：
```json
{"choices":[{"index":0,"delta":{"content":"你好"}}],"usage":null,"id":"chatcmpl-xxx","object":"chat.completion.chunk","created":...,"model":"..."}
```

**最终 usage chunk**（在 `data: [DONE]` 前）：
```json
...
- 空闲 session 每 `max_idle_seconds/2`（默认 150s，可通过 `[session_pool]` 下的 `max_idle_seconds` 配置）自动清理
